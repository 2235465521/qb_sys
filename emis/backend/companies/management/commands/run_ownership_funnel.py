...
执行“方案一：漏斗分层过滤法”（本地规则筛 90%+ 0成本，API 只查 10% 存疑）
"""
...
class Command(BaseCommand):
    help = '运行方案一：企业所有制漏斗分层过滤引擎（本地 0 成本高精度打标 90%+，API 精准穿透存疑 10%）'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"待处理企业总数: {total_count} 条 | 运行模式: {mode} | 批次大小: {batch_size}")
        self.stdout.write("--------------------------------------------------")
        ...
        if mode == 'scan':
            ...
            # 仅取必要字段加速
            company_records = qs.values('id', 'name', 'company_type')[:limit] if limit > 0 else qs.values('id', 'name', 'company_type')
            
            for c in company_records.iterator(chunk_size=batch_size):
                res = OwnershipTagService.funnel_classify(c['name'], c['company_type'])
                stats['processed'] += 1

                if res['tier'] == 1:
                    if 'private' in res['tag_codes']:
            ...
            tier1_total = stats['tier1_private'] + stats['tier1_foreign_hmt'] + stats['tier1_state_whitelist']
            ...
            self.stdout.write("\n" + self.style.SUCCESS("=== 企业所有制漏斗分层分析报告 ==="))
            self.stdout.write(f"[-] 扫描耗时: {duration} 秒 (平均处理速率: {int(stats['processed']/max(duration,0.01))} 条/秒)")
            self.stdout.write(f"[-] 扫描企业总数: {stats['processed']} 家")
            self.stdout.write("--------------------------------------------------")
            self.stdout.write(self.style.SUCCESS(f"【Tier 1: 本地 0 成本确定性命中】: {tier1_total} 家 ({tier1_ratio}%) -> 100% 规则确定，花费 0 元！"))
            self.stdout.write(f"   [+] 纯民营企业 (自然人控股/独资): {stats['tier1_private']} 家 ({round(stats['tier1_private']/stats['processed']*100, 2) if stats['processed'] > 0 else 0}%)")
            ...
            self.stdout.write("\n下一步操作指南：")
            self.stdout.write("   1. 运行 `python manage.py run_ownership_funnel --mode apply-tier1` 即刻完成这 90%+ 企业的 0 成本全量打标。")
        ...
        if mode in ['apply-tier1', 'all']:
            ...
            for company in company_objs.iterator(chunk_size=batch_size):
                processed += 1
                res = OwnershipTagService.funnel_classify(company.name, company.company_type)
                ...
                if len(batch_relations) >= 5000:
                    ...
                    batch_relations = []
                    self.stdout.write(f"已完成 {processed}/{total_count} 家企业打标...")

            if batch_relations:
                CompanyCategoryRelation.objects.bulk_create(batch_relations, ignore_conflicts=True)

            duration = round(time.time() - start_time, 2)
            self.stdout.write(self.style.SUCCESS(f"Tier 1 本地打标完成！成功为 {applied_count} 家企业写入所有制标签 (耗时 {duration} 秒, 花费 0 元)。"))
